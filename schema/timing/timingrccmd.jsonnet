...
local defs = import "definitions.jsonnet";

// A schema builder in the given path (namespace)
local ns = "dunedaq.timing.timingrccmd";
local s = moo.oschema.schema(ns);

local defsh = moo.oschema.hier(defs);
...
local timingrccmd = {
    ...
    timing_master_send_fl_cmd_cmd__payload: s.record("TimingMasterSendFLCmdCmdPayload",[
        s.field("fl_cmd_id", defsh.dunedaq.timing.definitions.FixedLengthCommandType,
            doc="ID of target endpoint"),
...
defs + moo.oschema.sort_select(timingrccmd, ns)
